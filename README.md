...
├── blur/          흐림 의심
├── eyes_closed/   눈 감음 의심
├── unknown/       판별 실패 (파일이 깨졌거나 처리 중 오류)
...
- 어떤 이유로든 판별하지 못한 사진은 `unknown/` 에 모입니다. 파일이 깨졌거나 형식이
  이상한 경우이니 직접 열어 확인하세요 (여기서도 파일은 지워지지 않습니다)
...
- **사진이 `Photos` 루트에 그대로 남아 있음** → 복사가 아직 안 끝난 파일입니다. 다음 실행 때 처리됩니다
  (처리하다 오류가 난 파일은 루트가 아니라 `unknown/` 으로 갑니다)
